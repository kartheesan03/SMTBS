...
const FinancialOperations = () => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    let monthSalesRev = salesOrders.filter(o => ["Delivered", "Completed", "Invoice Generated", "Workflow Completed"].includes(o.status)).filter(o => {
      const d = new Date(o.orderDate || o.createdAt);
      return !isNaN(d) && d.getMonth() === mIdx;
    }).reduce((s, o) => s + (Number(o.totalAmount) || Number(o.grandTotal) || 0), 0);
    let monthPurchaseExp = purchaseOrders.filter(o => {
      const d = new Date(o.orderDate || o.createdAt);
      return !isNaN(d) && d.getMonth() === mIdx;
    }).reduce((s, o) => s + (Number(o.totalAmount) || Number(o.grandTotal) || 0), 0);
    plData.push({
  ...
  const spendData = Object.entries(catSpendMap).sort((a, b) => b[1] - a[1]).map(([name, value], idx) => ({
    name,
    value,
    color: categoryColors[idx % categoryColors.length],
    pct: `${Math.round(value / totalSpendForCalc * 100)}%`
  }));
  const displaySpendData = spendData.length > 0 ? spendData : [{
    name: "No Data",
    value: 1,
    color: "#f1f5f9",
    pct: "0%"

...
const FinancialOperations = () => {
  ...
  const salesOrders = orders.filter((o) =>
    (o.orderType || "").toLowerCase().includes("sales")
  );
  const purchaseOrders = orders.filter((o) =>
    (o.orderType || "").toLowerCase().includes("purchase")
  );
  const completedSales = salesOrders.filter((o) =>
    [
      "Delivered",
      "Completed",
      "Invoice Generated",
  ...
  const totalPayables = purchaseOrders.reduce(
    (s, o) => s + (Number(o.totalAmount) || Number(o.grandTotal) || 0),
    0
  );
  ...
  const outstandingRecv = salesOrders
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    plData.push({
      name: mName,
      revenue: monthSalesRev,
      profit: monthSalesRev - monthPurchaseExp,
    });
  }
      ...
      o.items.forEach((item) => {
        ...
        catSpendMap[cat] =
          ...
          (Number(item.quantity) || 0) *
            (Number(item.price) || Number(item.unitPrice) || 0);
      });
    } else {
      const cat = "General";
      catSpendMap[cat] =
        (catSpendMap[cat] || 0) +
        (Number(o.totalAmount) || Number(o.grandTotal) || 0);
    }
  });
  const actualTotalSpend = Object.values(catSpendMap).reduce(
    (s, v) => s + v,
    0
  );
  ...
  const displaySpendData =
    spendData.length > 0
      ? spendData
      : [{ name: "No Data", value: 1, color: "#f1f5f9", pct: "0%" }];
  const invoiceData = orders.map((o, i) => {
    const isPurchase = (o.orderType || "").toLowerCase().includes("purchase");
    return {
      id:
        o.invoiceNumber ||
        (o._id ? String(o._id).substring(0, 8).toUpperCase() : "Pending"),
      type: isPurchase ? "Payable" : "Receivable",
      party: isPurchase
        ? o.vendor?.companyName || o.vendor?.name || "Vendor"
        : o.customer?.company || o.customer?.name || "Customer",
      poSo: o.orderNumber || "",
            ...
            >
              ...
              <thead>
                ...
                <tr>
                  ...
                  <th>PO/SO</th> <th style={{ textAlign: "right" }}>AMOUNT</th>{" "}
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((inv, i) => {
                    const statusColors = {
                      ...
                      Sent: "rd-status-blue",
                      Draft: "rd-status-gray",
                      Pending: "rd-status-orange",
                    };
                    const typeColor =
                      inv.type === "Receivable" ? "#10b981" : "#f59e0b";
                    const typeBg =
                      inv.type === "Receivable" ? "#ecfdf5" : "#fff7ed";
                    return (
                      <tr key={inv._id || i}>
                        {" "}
                        <td
                        ...
                        <td style={{ color: "#64748b" }} data-label="PO/SO">
                          {inv.poSo || "-"}
                        </td>{" "}
                        <td
                          style={{
                        ...
                        <td
                          style={{
                            color:
                              inv.status === "Overdue" ? "#ef4444" : "#64748b",
                        ...
                        <td data-label="Status">
                          <span
                            className={`ui-badge ${
                              statusColors[inv.status] === "rd-status-red"
                                ? "danger"
                                : statusColors[inv.status] === "rd-status-green"
                                ? "success"
                                : statusColors[inv.status] ===
                                  "rd-status-orange"
                                ? "warning"
                                : statusColors[inv.status] === "rd-status-blue"
                                ? "primary"
                                : "gray"
                            }`}
                          >

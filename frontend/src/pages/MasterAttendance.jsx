...
const DailyTab = ({ canEdit }) => {
  ...
  const departments = [
    "All",
    ...new Set(data.map((r) => r.employee?.department).filter(Boolean)),
  ];
  const filtered = data.filter((r) => {
    ...
    if (statusFilter !== "All") {
      if (statusFilter === "Present")
        mst = r.status === "Present" || r.status === "Late";
  ...
  const exportCSV = () => {
    ...
    filtered.forEach((r) => {
      ...
      rows.push([
        ...
        calcHours(r.checkIn, r.checkOut, r.date),
        r.status || "-",
      ]);
    });
...
const MonthlyTab = () => {
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    a.download = `monthly_summary_${year}_${String(month).padStart(
      2,
      "0"
    )}.csv`;
    a.click();
  ...
  const avgRate =
    filtered.length > 0
      ? filtered.reduce((s, r) => s + (r.rate || 0), 0) / filtered.length
      : 0;
  return (
    <>
      {/* Summary KPIs */}
      <div style={{ marginBottom: 24 }}>
        <StatsGrid>
          ...
          <StatsCard
            ...
            value={filtered.filter((r) => r.absent === 0).length}
            colorTheme="purple"
            icon={CheckCircle}
            trendValue="No absences"
            trendPositive={true}
          />
          <StatsCard
            title="Needs Attention"
            value={filtered.filter((r) => (r.rate || 0) < 0.75).length}
            colorTheme="pink"
            icon={AlertCircle}
            trendValue="Below 75% rate"
            trendPositive={false}
          />
        </StatsGrid>
      </div>
                ...
                paginated.map((r, i) => {
                  ...
                  return (
                    <tr key={r._id || i}>
                      ...
                      <td style={{ fontWeight: 500 }} data-label="Check Out">
                        {formatTime(r.checkOut)}
                      </td>
                      <td data-label="Hours">
                        <span style={{ fontWeight: 700, color: "#3b82f6" }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid #e2e8f0",
                    borderRadius: 0,
                    background: p === page ? "#1e293b" : "#f8fafc",

...
const OrderDetails = () => {
  ...
  const handleEmployeeCheck = async (action) => {
    try {
      const { data } = await API.put(`/orders/${id}/employee-check`, {
        action,
      });
      setOrder(data);
      if (action === "low_stock")
        toast.success("Manager notified about low stock");
      else toast.success("Order approved and sent to Sales");
    } catch (err) {
      toast.error(
        err.response?.data?.message || "Failed to process order check"
      );
    }
  };
  ...
  const handleDownloadInvoice = () => {
    ...
    let billToPhone = isPurchase
      ? order.vendor?.phone || order.vendorPhone || ""
      : order.customer?.phone || order.customerPhone || "";
    let billToAddress = isPurchase
      ? order.vendor?.address || order.vendorAddress || ""
    ...
    if (order.items && order.items.length > 0) {
      order.items.forEach((item) => {
        let materialName =
          item.material?.name || item.materialName || "Material";
    ...
    doc.setFontSize(12);
    doc.text(
      `Grand Total: Rs. ${(order.totalAmount || grandTotal).toLocaleString()}`,
      14,
    ...
    >
      <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
        <div
          style={{
            width: "64px",
            height: "64px",
            background: isSales ? "#e0e7ff" : "#dcfce7",
            borderRadius: "0px",
            color: isSales ? "#4f46e5" : "#10b981",
            display: "flex",
            alignItems: "center",
        ...
        <div>
          ...
          >
            ...
            <span style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              {isSales ? <User size={14} /> : <Building2 size={14} />}{" "}
              {entityName}
            </span>
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
        ...
        {(userRole === "manager" || userRole === "admin") &&
          order.approvalStatus === "Pending Manager Approval" && (
            <button
  ...
  const sidebarContent = (
    <div className="standard-section">
      <h4 style={{ margin: "0 0 16px 0", fontSize: "16px", color: "#1e293b" }}>
        {isSales ? "Customer" : "Vendor"} Information
      </h4>
  ...
  const itemColumns = [
    {
      key: "material",
      label: "Material / Item",
      render: (val, row) => val?.name || row.materialName || "Unknown Material",
    },
    ...
    },
    {
      ...
      label: "Unit Price",
      render: (val, row) =>
        `₹${(val || row.material?.price || 0).toLocaleString()}`,
    },
    {
      key: "total",
      label: "Total",
      render: (_, row) =>
        `₹${(
          (row.price || row.material?.price || 0) * row.quantity
        ).toLocaleString()}`,
    },
  ];
  return (
    ...
    >
      <div className="standard-section">
        ...
        <div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
          ...
          >
            ...
            >
              <ShoppingCart size={16} /> Total Items
            </div>
            <div
              style={{ fontSize: "28px", fontWeight: "bold", color: "#0f172a" }}
            >
              {items.length}
            </div>
          </div>
      ...
      {(order.status === "Out for Delivery" ||
        (liveData && liveData.liveLocation)) && (
        <div className="standard-section">
          <div
            className="standard-section-header"
            style={{
          ...
          >
            ...
            {liveData?.trackingStatus !== "Delayed" &&
              liveData?.trackingStatus !== "Delivered" && (
                <button
                  onClick={handleFlagDelayed}
                  style={{
                    padding: "6px 12px",
                    background: "#fef2f2",
                    color: "#ef4444",
          ...
          >
            {liveData?.liveLocation ? (
              <MapContainer
                center={[liveData.liveLocation.lat, liveData.liveLocation.lng]}
                zoom={14}
              ...
              >
                ...
                <Marker
                  position={[
                    liveData.liveLocation.lat,
                    liveData.liveLocation.lng,
                  ]}
                  icon={truckIcon}
                >
                  <Popup>
                    <strong>{liveData.trackingStatus}</strong>
                    <br />
                    ETA:{" "}
                    {liveData.deliveryETA
                      ? new Date(liveData.deliveryETA).toLocaleTimeString()
                      : "N/A"}
                    <br />

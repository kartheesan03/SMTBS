...
const s = {
  ...
  leftPanel: {
    background:
      "linear-gradient(160deg, #4F46E5 0%, #6366f1 50%, #818cf8 100%)",
    padding: "36px 28px",
    display: "flex",
...
const GeneratePayroll = () => {
  ...
  const selectedEmployee = useMemo(() => {
    ...
    return (
      employees.find(
        (emp) => String(emp._id || emp.id) === formData.employeeId
      ) || null
    );
  ...
  const handleCalculateDeductions = async () => {
    if (!formData.employeeId || !formData.month || !formData.basicSalary) {
      return showToast(
        "Please select an employee and enter basic salary first",
        false
      );
    }
    setCalculating(true);
    try {
      ...
      showToast(
        `Calculated: ${data.absentDays} Absents, ${data.lateDays} Lates`
      );
    } catch (err) {
      showToast(
        err.response?.data?.message || "Error calculating deductions",
        false
      );
    } finally {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.message || "Error generating payroll",
        false
      );
      setSubmitting(false);
    }
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "20px", maxWidth: "960px" }}>
          <PageHeader
            title="Generate New Payroll Entry"
            badge="HRMS"
            subtitle="Create employee salary record for selected month"
            showBack={true}
            backPath="/payroll"
          />
        </div>
        ...
        <div style={s.splitGrid}>
          ...
          <div style={s.rightPanel}>
            ...
            <div style={s.formSubtitle}>
              Fill in the compensation for this period
            </div>
            <form
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selectedEmp = employees.find(
                        (emp) => String(emp._id || emp.id) === e.target.value
                      );
                      setFormData({
                        ...formData,
                        employeeId: e.target.value,
                        basicSalary:
                          selectedEmp?.salary || formData.basicSalary,
                      });
                      setCalcStats(null);
                    }}
                    style={{
                  ...
                  >
                    <div style={s.calcStatsGrid}>
                      ...
                      <div style={s.calcStat("#f59e0b")}>
                        ...
                        <strong
                          style={{ color: "var(--warning)", fontSize: "16px" }}
                        >
                          {calcStats.lateDays}
                        </strong>
                      </div>

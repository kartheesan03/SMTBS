          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div>
              ...
              <input
                ...
                value={form.color}
                onChange={handle}
                style={{ ...inp, padding: 4, height: 36, width: 60 }}
              />
            </div>
            <label
              style={{
...
const HolidayCalendar = () => {
  ...
  const handleSave = (saved, action) => {
    if (action === "add")
      ...
      setHolidays((prev) => prev.map((h) => (h.id === saved.id ? saved : h)));
  };
              ...
              {Object.entries(TYPE_CONFIG).map(([type, cfg]) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#94a3b8" }}>
                      {holidays.filter((h) => h.type === type).length} holiday
                      {holidays.filter((h) => h.type === type).length !== 1
                        ? "s"
                        : ""}
                    </div>
                  </div>
                ...
                {["All", ...Object.keys(TYPE_CONFIG)].map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      border:
                        filterType === t
                          ? `1.5px solid ${TYPE_CONFIG[t]?.color || "#6366f1"}`
                          : "1.5px solid #e2e8f0",
                      background:
                        filterType === t
                          ? TYPE_CONFIG[t]?.bg || "#eef2ff"
                          : "#fff",
                      color:
                        filterType === t
                          ? TYPE_CONFIG[t]?.color || "#4f46e5"
                          : "#64748b",
                  ...
                  {loading ? (
                    ...
                    Object.entries(grouped).map(([monthIdx, mHols]) => (
                      <React.Fragment key={monthIdx}>
                        ...
                        {mHols.map((h) => {
                          const cfg =
                            TYPE_CONFIG[h.type] || TYPE_CONFIG.Company;
                          const d = new Date(h.date);

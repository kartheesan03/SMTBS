...
import { Package, Truck, CheckCircle, IndianRupee, Search, Plus, Eye, ArrowRight, Activity, TrendingUp, ShoppingCart } from "lucide-react";
...
const OrderManagement = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      let extractedOrders = [];
      if (Array.isArray(data)) extractedOrders = data;else if (data && Array.isArray(data.orders)) extractedOrders = data.orders;else if (data && Array.isArray(data.data)) extractedOrders = data.data;
  ...
  const statusDistData = [{
    name: "Created",
    value: filteredOrdersForStatus.filter(o => ["New", "Created", "Pending"].includes(o.status)).length || 0,
    fill: "#3b82f6"
  }, {
    name: "Manager/Admin",
    value: filteredOrdersForStatus.filter(o => ["Pending Approval", "Manager/Admin Review"].includes(o.status)).length || 0,
    fill: "#f59e0b"
  }, {
    name: "Verification",
    value: filteredOrdersForStatus.filter(o => ["Employee Verification", "Awaiting Stock Check"].includes(o.status)).length || 0,
    fill: "#8b5cf6"
  }, {
    name: "Stock Issue",
    value: filteredOrdersForStatus.filter(o => ["Low Stock", "Low Stock Hold", "Out of Stock", "Waiting for Manager"].includes(o.status)).length || 0,
    fill: "#ef4444"
  }, {
    name: "Processing",
    value: filteredOrdersForStatus.filter(o => ["Sales Processing", "Inventory Verified", "Ready for Delivery", "Processing", "In Transit"].includes(o.status)).length || 0,
    fill: "#f59e0b"
  }, {
    name: "Out for Delivery",
    value: filteredOrdersForStatus.filter(o => o.status === "Out for Delivery").length || 0,
    fill: "#0ea5e9"
  }, {
    name: "Delivered",
    value: filteredOrdersForStatus.filter(o => ["Delivered", "Invoice Generated", "Completed", "Workflow Completed"].includes(o.status)).length || 0,
    fill: "#10b981"
  }];
  ...
  return <motion.div initial={{
    ...
    duration: 0.4
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          {" "}
          <StatsCard title="Total Orders" value={orders.length} colorTheme="purple" icon={Package} trendValue="Combined sales & purchases" trendPositive={true} />{" "}
          <StatsCard title="Delivered" value={deliveredOrders.length} colorTheme="mint" icon={CheckCircle} trendValue="Successfully completed" trendPositive={true} />{" "}
          <StatsCard title="Sales Revenue" value={formatCurrency(salesRevenue)} colorTheme="blue" icon={IndianRupee} trendValue="Total incoming" trendPositive={true} />{" "}
          <StatsCard title="Purchase Cost" value={formatCurrency(purchaseCost)} colorTheme="peach" icon={ShoppingCart} trendValue="Total outgoing" trendPositive={false} />{" "}
        </StatsGrid>{" "}
        {/* Charts */}{" "}
        <motion.div initial={{
        opacity: 0,
        ...
        marginBottom: 24
          ...
          flex: 1
            ...
            marginBottom: 20
              ...
              <select value={statusPeriod} onChange={e => setStatusPeriod(e.target.value)} style={{
              padding: "4px 10px",
              borderRadius: 0,
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                ...
                <BarChart data={statusDistData} barSize={24}>
                  ...
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    {" "}
                    {statusDistData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}{" "}
                  </Bar>{" "}
                </BarChart>{" "}
              </ResponsiveContainer>{" "}
            </div>{" "}
          ...
          flex: 1.5
            ...
            marginBottom: 20
              ...
              <select value={revenuePeriod} onChange={e => setRevenuePeriod(e.target.value)} style={{
              padding: "4px 10px",
              borderRadius: 0,
              border: "1px solid #e2e8f0",
              fontSize: 12,
          ...
          gap: 16
            ...
            flexWrap: "wrap"
              ...
              alignItems: "center"
                ...
                {" "}
                {writeAccess && <button className="rd-btn-solid" onClick={() => navigate(`/orders/create/${window.location.pathname.includes("purchase") ? "purchase" : "sales"}`)}>
                    + New Order
                  </button>}{" "}
              </div>{" "}
            </div>{" "}
              ...
              <tbody>
                ...
                return <tr key={o._id || o.id || i} style={{
                  cursor: "pointer"
                    ...
                    whiteSpace: "nowrap"
                  }} title={orderId} data-label="Order ID">
                          {orderId}
                        </td>{" "}
                        <td data-label="Type">
                          {" "}
                      ...
                      fontSize: 11,
                      fontWeight: 700,
                      background: type === "Purchase" ? "#fff7ed" : "#f0fdf4",
                      color: type === "Purchase" ? "#ea580c" : "#16a34a"
                    }}>
                            {" "}
                            {type.toUpperCase()}{" "}
                          </span>{" "}
                    ...
                    textOverflow: "ellipsis",
                    whiteSpace: "nowrap"
                  }} title={partyName} data-label="Customer/Vendor">
                          {partyName}
                        </td>{" "}
                        <td style={{
                    fontWeight: 700,

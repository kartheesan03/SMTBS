...
const OrderTracking = () => {
  ...
  useEffect(() => {
    ...
    if (order && order.workflow) {
      const activeIndex = order.workflow.findIndex(
  ...
  const fetchData = async () => {
    try {
      ...
      const foundOrder = data.find(
        (o) => o.id?.toString() === orderId || o._id?.toString() === orderId
      );
      if (foundOrder) {
        setOrder(foundOrder);
        if (foundOrder.items) {
          ...
          foundOrder.items.forEach((i) => {
            const id =
              i.materialId || (i.material ? i.material.id : i.material);
      ...
      try {
        ...
        if (auditRes.data && Array.isArray(auditRes.data)) {
          const filteredLogs = auditRes.data.filter(
            (log) => log.targetId?.toString() === orderId
          );
          setAuditLogs(filteredLogs);
        }
      } catch (auditErr) {
        console.log(
          "Could not fetch audit logs (possibly restricted role): ",
          auditErr
        );
      }
      try {
        ...
        if (empRes.data) {
          setEmployees(
            empRes.data.filter(
              (u) => u.role !== "Customer" && u.role !== "Guest"
            )
          );
        }
      } catch (empErr) {
  ...
  const handleManagerResolveStock = async (resolution) => {
    ...
    try {
      setSubmitting(true);
      await API.post(`/orders/${orderId}/manager-resolution`, {
        resolution,
        remarks,
      });
  ...
  if (error || !order) {
    return (
      <div
        ...
        style={{ minHeight: "100vh", flexDirection: "column", gap: "16px" }}
      >
  ...
  const hasIssue =
    ...
    (order.trackingTimeline &&
      order.trackingTimeline.some((t) =>
        ["Low Stock", "Out Of Stock"].includes(t.status)
      ));
  const isPurchase =
  ...
  const generateFallbackWorkflow = (status, isPurch) => {
    const baseStages = [
      ...
      {
        stage: isPurch ? "Purchase Processing" : "Sales Processing",
        role: isPurch ? "Vendor" : "Sales",
      },
      {
        stage: isPurch ? "Material Dispatch" : "Out for Delivery",
        role: isPurch ? "Vendor" : "Sales",
      },
      {
        stage: isPurch ? "Material Received" : "Delivered",
        role: "Admin/Employee",
      },
    ...
    if (
      ...
      ["out for delivery", "shipped", "dispatched"].some((x) => s.includes(x))
    )
      activeIdx = 5;
    else if (
      ["delivered", "completed", "material received"].some((x) => s.includes(x))
    )
      activeIdx = 7;
    ...
    return baseStages.map((stage, i) => {
      ...
      if (
        ["cancelled", "rejected"].some((x) => s.includes(x)) &&
        i === activeIdx
      ) {
        st = "Rejected";
      }
  ...
  const defaultWorkflowFallback = generateFallbackWorkflow(
    currentStatus,
    isPurchase
  );
  const workflow =
  ...
  const RoleActionPanel = ({
    ...
    const renderEmptyPanel = () => (
      ...
      >
        <p style={{ color: "#475569", margin: 0, fontWeight: 500 }}>
          ...
          />
          Order is currently with <strong>{currentActiveRole}</strong> for{" "}
          <strong>{currentStage}</strong>. No action needed from you.
        </p>
      </div>
    );
    if (isCancelled || currentStatus === "Workflow Completed") {
      return (
        ...
        >
          <p style={{ margin: 0 }}>
            No further actions can be taken on this order. Workflow is
            concluded.
          </p>
          {currentStatus === "Workflow Completed" && (
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                navigate(`/invoice/${order.id || order._id}`);
              }}
            >
              View / Generate Bill
            </button>
    ...
    if (
      ...
      currentStage === "Invoice Generated"
    ) {
      return (
        <div className="action-form">
          <div className="button-group">
            ...
            <button
              className="erp-btn btn-success"
              disabled={submitting}
              onClick={() =>
                handleActionClick("COMPLETE", "Workflow Completed")
              }
            >
    ...
    if (loggedInRole === "Manager" || loggedInRole === "Admin") {
      if (
        ["Created", "Pending Approval", "Draft"].includes(currentStatus) ||
        currentStatus === "Manager/Admin Review" ||
        currentStatus === "Order Created" ||
        currentStage === "Admin/Manager Review"
      ) {
        return (
          <div className="action-form">
            ...
            <div className="button-group">
              <button
                ...
                disabled={submitting}
                onClick={() =>
                  handleActionClick("APPROVE", "Employee Verification")
                }
              >
                {submitting ? "Processing..." : "Approve Order"}
              </button>
              <button
                className="erp-btn btn-warning"
                disabled={submitting}
                onClick={() => handleActionClick("MODIFY", "Draft")}
              >
                Request Modification
              </button>
              <button
                className="erp-btn btn-danger"
                disabled={submitting}
                onClick={() => handleActionClick("REJECT", "Rejected")}
              >
                Reject Order
              </button>
            </div>
      ...
      if (
        ["Low Stock", "Out of Stock", "Waiting for Manager"].includes(
          currentStatus
        )
      ) {
        return (
          <div className="action-form">
            <h4 style={{ margin: "0 0 16px 0", color: "#d97706" }}>
              ⚠ Stock Issue Reported
            </h4>
            <p style={{ margin: "0 0 16px 0", color: "#64748b" }}>
              The employee reported an inventory shortage for this order.
            </p>
            <div className="form-group">
            ...
            <div className="button-group">
              <button
                className="erp-btn btn-primary"
                disabled={submitting}
                onClick={() => handleManagerResolveStock("Allocate")}
              >
              ...
              <button
                className="erp-btn btn-success"
                disabled={submitting}
                onClick={() => handleManagerResolveStock("Approve Purchase")}
              >
    ...
    if (loggedInRole === "Employee") {
      if (
        ...
        currentStage === "Employee Verification"
      ) {
        const allVerified = Object.values(itemsVerification).every(
          (v) => v.status === "In Stock"
        );
        const hasIssue = Object.values(itemsVerification).some((v) =>
          ["Low Stock", "Out of Stock"].includes(v.status)
        );
        return (
          <div
            className="action-form inventory-verify-form"
            style={{
              width: "100%",
              maxWidth: "none",
              background: "#fff",
          ...
          >
            ...
            >
              Please physically verify the materials in the warehouse before
              proceeding. Select the actual stock status for each item.
            </p>
              ...
              >
                ...
                <tbody>
                  {(order.items || []).map((item, idx) => {
                    ...
                    const vState = itemsVerification[matId] || {
                      status: "In Stock",
                      remarks: "",
                    };
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "12px", color: "#64748b" }}>
                          {mat.warehouse
                            ? `${mat.warehouse} / ${mat.shelf || "No Shelf"}`
                            : "Main Warehouse"}
                        </td>
                        <td
                          style={{
                        ...
                        <td style={{ padding: "12px" }}>
                          <select
                            value={vState.status}
                            onChange={(e) =>
                              setVState("status", e.target.value)
                            }
                            style={{
                              padding: "6px 12px",
                              borderRadius: "0px",
                              ...
                              backgroundColor:
                                vState.status === "In Stock"
                                  ? "#ecfdf5"
                                  : vState.status === "Low Stock"
                                  ? "#fffbeb"
                                  : "#fef2f2",
                              color:
                                vState.status === "In Stock"
                        ...
                        <td style={{ padding: "12px" }}>
                          <input
                            ...
                            onChange={(e) =>
                              setVState("remarks", e.target.value)
                            }
                            style={{
                              padding: "6px 12px",
                              width: "100%",
        ...
        return (
          <div className="action-form">
            ...
            <p style={{ margin: "0 0 24px 0", color: "#64748b" }}>
              All materials are ready for processing. Please provide final
              approval to send this order to Sales.
            </p>
            <button
              className="erp-btn btn-success"
              disabled={submitting}
              onClick={handleEmployeeFinalApprove}
            >
              {submitting ? "Processing..." : "Approve Order"}
            </button>
          </div>
          ...
          <div className="action-form">
            ...
            <div className="button-group">
              <button
                className="erp-btn btn-primary"
                disabled={submitting}
                onClick={() => handleActionClick("PACK", "Packing")}
              >
          ...
          <div className="action-form">
            <div className="button-group">
              <button
                ...
                disabled={submitting}
                onClick={() =>
                  handleActionClick("START_DELIVERY", "Out for Delivery")
                }
              >
          ...
          <div className="action-form">
            <div className="button-group">
              <button
                className="erp-btn btn-success"
                disabled={submitting}
                onClick={() => handleActionClick("DELIVER", "Delivered")}
              >
          ...
          <div className="action-form">
            <div className="button-group">
              <button
                ...
                disabled={submitting}
                onClick={() =>
                  handleActionClick("GENERATE_INVOICE", "Invoice Generated")
                }
              >
    ...
    if (loggedInRole === "Vendor" || loggedInRole === "Vendor User") {
      if (
        ["Purchase Required", "Manager/Admin Review"].includes(currentStatus) ||
        currentStatus === "Approved" ||
        currentStage === "Purchase Required"
      ) {
        return (
          <div className="action-form">
            <div className="form-group">
            ...
            <div className="button-group">
              <button
                className="erp-btn btn-success"
                disabled={submitting}
                onClick={() => handleActionClick("ACCEPT", "Vendor Accepted")}
              >
              ...
              <button
                className="erp-btn btn-danger"
                disabled={submitting}
                onClick={() => handleActionClick("REJECT", "Vendor Rejected")}
              >
                Reject Purchase
              </button>
            </div>
          ...
          <div className="action-form">
            <div className="button-group">
              <button
                ...
                disabled={submitting}
                onClick={() =>
                  handleActionClick("DISPATCH", "Material Received")
                }
              >
    ...
    >
      ...
      >
        <PageHeader
          ...
          badge="ERP"
          subtitle="Manage and track the complete order lifecycle"
        />
        <button
          className="erp-btn btn-outline"
          onClick={() => navigate(-1)}
      ...
      <div className="erp-layout">
        ...
        <section className="erp-card summary-section">
          ...
          <div className="summary-grid">
            ...
            <div className="grid-item">
              ...
              <span className="value">
                {order.employeeId ? `#${order.employeeId}` : "Unassigned"}
              </span>
            </div>
            <div className="grid-item">
              <span className="label">Delivery Type</span>
              <span className="value">
                {order.orderType === "sales" ? "Sales Delivery" : "Standard"}
              </span>
            </div>
            <div className="grid-item">
              <span className="label">Created Date</span>
              <span className="value">
                {formatDateOnly(order.createdAt || order.orderDate)}
              </span>
            </div>
            <div className="grid-item">
          ...
          <div className="workflow-stepper-container">
            <div className="workflow-stepper">
              {workflow.map((stageObj, index) => {
                ...
                const isPopoverOpen =
                  activePopover === index ||
                  (activePopover === null && statusClass === "current");
                return (
                  <div
                  ...
                  >
                    ...
                    {/* Popover Tooltip */}
                    <div
                      className={`step-popover ${isPopoverOpen ? "open" : ""}`}
                    >
        ...
        <div className="erp-grid-2col">
          ...
          <section
            className="erp-card notifications-section"

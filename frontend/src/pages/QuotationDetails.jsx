...
const QuotationDetails = () => {
  ...
  const handleConvertToOrder = async () => {
    if (
      !window.confirm(
        "Are you sure you want to convert this quotation into a Sales Order?"
      )
    )
      return;
    setConverting(true);
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(quotation.customer?.company || quotation.customer?.name || quotation.customerName || "Customer", 14, 62);
    ...
    quotation.items.forEach((item) => {
      const rowData = [
        item.material?.name || item.materialName || "Unknown Item",
        item.quantity,
  ...
  return (
    <PageContainer>
      ...
      <DetailViewContainer>
        <ProfileHeader
          title={quotation.customer?.company || quotation.customer?.name || quotation.customerName}
          subtitle={`Generated by ${quotation.createdByName || "System"}`}
          avatarText={(quotation.customer?.company || quotation.customer?.name || quotation.customerName).substring(0, 2).toUpperCase()}
          badges={[
            {
        ...
        <div className="ui-grid-2" style={{ padding: "24px" }}>
          <KeyValueCard
            ...
            items={[
              {
                label: "Issue Date",
                value: new Date(quotation.date).toLocaleDateString(),
              },
              {
                label: "Valid Until",
              ...
              <table className="ui-table">
                ...
                <tbody>
                  {quotation.items.map((item, i) => (
                    <tr key={i}>
                      <td>{item.material?.name || item.materialName || "Unknown Item"}</td>
                      <td>{item.quantity}</td>

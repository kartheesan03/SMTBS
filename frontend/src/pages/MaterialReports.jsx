...
const MaterialReports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        setLoading(true);
        const [matsRes, movesRes] = await Promise.all([API.get("/materials"), API.get("/materials/movements/all").catch(() => ({
          data: []
        }))]);
  ...
  const columns = [{
    ...
    render: (_, row) => <span style={{
      ...
      color: "#10b981"
          ...
          $
          {((row.quantity || 0) * (row.price || 0)).toLocaleString(undefined, {
  ...
  const exportToCSV = () => {
    ...
    materials.forEach(row => {
      ...
      const values = [`"${row.name}"`, `"${row.sku || ""}"`, `"${row.category || ""}"`, stock, row.price || 0, totalValue, status];
  ...
  return <div className="rd-container">
      <div className="rd-content">
        ...
        })}`} colorTheme="mint" icon={TrendingUp} trendValue="Across all materials" trendPositive={true} />
          <StatsCard title="Unique Materials" value={summary.items.toLocaleString()} colorTheme="blue" icon={Package} trendValue="Tracked SKUs" trendPositive={true} />
          <StatsCard title="Low Stock Items" value={summary.lowStock} colorTheme="peach" icon={AlertTriangle} trendValue="Requires restocking" trendPositive={false} />
          <StatsCard title="Total Movements" value={summary.movementsCount.toLocaleString()} colorTheme="purple" icon={Clock} trendValue="Recorded transactions" trendPositive={true} />
        </StatsGrid>
        <div style={{
        marginTop: "24px"
      }}>

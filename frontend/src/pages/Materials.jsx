...
const Materials = () => {
      ...
      render: (val, row) => (
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          <span style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>
            {row.movementsCount || 0} movements recorded
          </span>
        </div>
      ),
    },
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
          {writeAccess && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                navigate(`/materials/${row._id || row.id}`);
              }}
            >
              Edit
            </button>
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(
                `/tracking-overview?search=${encodeURIComponent(
                  row.sku || row.name
                )}`
              );
            }}
  ...
  const inStock = materialsData.filter(
    (m) => getComputedStatus(m) === "In Stock"
  ).length;
  const lowStock = materialsData.filter(
    (m) => getComputedStatus(m) === "Low Stock"
  ).length;
  const outOfStock = materialsData.filter(
                  ...
                  <div>
                    ...
                    <div style={{ marginBottom: 4, display: "flex", gap: 8 }}>
                      ...
                      <span style={{ fontWeight: 600, color: "#334155" }}>
                        {relativeTime(row.locationUpdatedAt)}
                      </span>
                    </div>

...
const MyTasks = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        broadcastRoles: formData.isBroadcast
          ? formData.broadcastRoles
          : undefined,
      };
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchesFilter =
      filter === "All" ||
      (isManager
        ? completions.some((c) => c.status === filter)
        : userStatus === filter);
    const matchesSearch =
  ...
  const pendingCount = allCompletions.filter(
    (c) => c.status === "Pending"
  ).length;
  const inProgressCount = allCompletions.filter(
  ...
  return (
    ...
    >
      ...
      >
        {loading ? (
          <div className="flex-center p-50" style={{ minHeight: "100vh" }}>
            <Loader size={30} className="spin-icon" />
          </div>
                  ...
                  <tr key={t._id || t.id}>
                        ...
                        <div className="assignee-list">
                          {completions.slice(0, 3).map((c, i) => {
                            ...
                            const userObj = allUsers.find(
                              (u) => String(u._id || u.id) === String(uId)
                            );
                            const uName = userObj
                              ? userObj.name
                              : c.user?.name || `User #${uId}`;
                    ...
                    <td style={{ width: "15%", minWidth: "120px" }}>
                      {isManager ? (
                        ...
                        <div
                          className={`status-pill ${myStatus
                            .toLowerCase()
                            .replace(" ", "-")}`}
                        >
                          {myStatus === "Pending" ? (
                      ...
                      <div className="action-btns">
                        {!isManager && myStatus !== "Completed" && (
                          <button
                            ...
                            onClick={() =>
                              handleUpdateStatus(t._id || t.id, myStatus)
                            }
                          >
          ...
          <div className="premium-card modal-content-lg animate-pop">
            ...
            <form onSubmit={handleSubmit} className="modal-form">
                ...
                <div className="form-group">
                  ...
                  <div className="toggle-group">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setFormData({ ...formData, isBroadcast: true })
                      }
                    >
              ...
              {formData.isBroadcast ? (
                <div className="form-group">
                  ...
                  <div className="role-chips">
                    {["Employee", "Sales"].map((role) => (
                      <button
                        ...
                        key={role}
                        className={`role-chip ${
                          formData.broadcastRoles.includes(role) ? "active" : ""
                        }`}
                        onClick={() => toggleBroadcastRole(role)}
                      >
                        {formData.broadcastRoles.includes(role) ? (
                          <CheckCircle2 size={14} />
                        ) : (
                          <User size={14} />
                        )}
                  ...
                  <p className="form-hint">
                    Task will be sent to all users with selected roles
                  </p>
                </div>
              ) : (
                <div className="form-group">
                  ...
                  <div className="assignee-selector">
                        ...
                        {employees.map((emp) => (
                          ...
                          >
                            <input
                              ...
                              onChange={() =>
                                toggleAssignee(String(emp._id || emp.id))
                              }
                            />
                        ...
                        {salesUsers.map((su) => (
                          ...
                          >
                            <input
                              ...
                              onChange={() =>
                                toggleAssignee(String(su._id || su.id))
                              }
                            />
                            <span className="assignee-avatar sales">
                              {(su.name || "?")[0]}
                            </span>
                            <span>{su.name}</span>
                          </label>
                    ...
                    {employees.length === 0 && salesUsers.length === 0 && (
                      ...
                      >
                        No employees or sales users found.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="notification-info glass-card">
                <Bell size={16} />
                <span>
                  Notifications will be sent to all assigned users automatically
                </span>
              </div>
              <div className="modal-actions">
                ...
                <button
                  ...
                  disabled={
                    submitting ||
                    (!formData.isBroadcast && formData.assignedTo.length === 0)
                  }
                >
                  <Send size={16} />{" "}

...
const MyAttendance = () => {
  ...
  const parseDateTime = (timeStr, baseDateStr) => {
    ...
    if (
      timeStr.includes("T") ||
      (timeStr.includes("-") && timeStr.includes(":") && timeStr.length > 10)
    ) {
      const d = new Date(timeStr);
      if (!isNaN(d.getTime())) return d;
    }
    const datePart = baseDateStr
      ? baseDateStr.split("T")[0]
  ...
  useEffect(() => {
    ...
    if (status && status.checkIn && !status.checkOut) {
      ...
      if (start && end) {
        ...
        const minutes = Math.floor((diff % 3600000) / 60000);
        setTimer(
          `${hours.toString().padStart(2, "0")}h ${hours
            .toString()
            .padStart(2, "0")}m 00s`
        );
  ...
  const fetchData = async () => {
    try {
      ...
      if (
        !historyRes.data.some((h) => h.date && h.date.split("T")[0] === today)
      ) {
        finalHistory = [
          { date: today, status: statusRes.data?.status || "-" },
          ...historyRes.data,
        ];
      }
      setHistory(finalHistory);
      if (historyRes.data.length > 0) {
        const presentDays = historyRes.data.filter(
          (h) => h.status === "Present" || h.status === "Late"
        ).length;
        const lateDays = historyRes.data.filter(
          (h) => h.status === "Late"
        ).length;
        const withHours = historyRes.data.filter(
          (h) => h.checkIn && h.checkOut
        );
        let totalHours = 0;
        withHours.forEach((h) => {
        ...
        const rate = Math.round(
          (presentDays / (historyRes.data.length || 1)) * 100
        );
        setStats((prev) => ({
          ...prev,
          present: presentDays,
        ...
        <div className="att-left-col">
          ...
          <div className="att-card att-hero-card">
            ...
            <div className="att-hero-mid">
              ...
              <div className="att-hero-goal">
                ...
                <div className="att-hero-goal-header">
                  {" "}
                  <span>Today's Goal</span> <span>{progressPercent}%</span>{" "}
                </div>{" "}
            ...
            <div className="att-hero-bottom">
              ...
              {!status?.checkIn ? (
                <button
                  className="att-hero-btn primary"
                ...
                <button className="att-hero-btn outline" disabled>
                  {" "}
                  <CheckCircle size={16} className="text-green" /> Shift
                  Completed{" "}
                </button>
              )}{" "}
              <button
                className="att-hero-btn outline"
                style={{
                  marginTop: !isShiftActive && !isShiftCompleted ? 0 : 8,
                }}
              >
                {" "}
          ...
          <div className="att-card" style={{ marginTop: 24 }}>
            ...
            <div className="att-feed-item">
              {" "}
              <div className="att-feed-icon text-green">
                <CheckCircle size={14} />
              </div>{" "}
              <div className="att-feed-content">
                {" "}
                <p>Checked In</p> <span>Today, 08:58 AM</span>{" "}
              </div>{" "}
            </div>{" "}
            <div className="att-feed-item">
              {" "}
              <div className="att-feed-icon text-amber">
                <Coffee size={14} />
              </div>{" "}
              <div className="att-feed-content">
                {" "}
                <p>Break Started</p> <span>Today, 10:45 AM</span>{" "}
              </div>{" "}
            </div>{" "}
            <div className="att-feed-item">
              {" "}
              <div className="att-feed-icon text-indigo">
                <CheckCircle size={14} />
              </div>{" "}
              <div className="att-feed-content">
                {" "}
                <p>Leave Approved</p> <span>Yesterday, 03:20 PM</span>{" "}
              </div>{" "}
            </div>{" "}
          </div>{" "}
        </div>{" "}
        ...
        <div className="att-right-col">
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <StatsGrid columns={4}>
              ...
              <StatsCard
                ...
                trendValue="Extra hours"
                icon={Activity}
                colorTheme="blue"
                trendPositive={true}
              />{" "}
              <div
                className="ent-module-card"
                style={{
          ...
          >
                ...
                {isShiftCompleted && (
                  <div className="att-timeline-item">
                    ...
                    <span className="att-timeline-time">
                      {formatTime(status?.checkOut, status?.date)}
                    </span>{" "}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div className="att-card">
                {" "}
                <h3 className="att-section-title">
                  <Target size={18} /> Work Progress
                </h3>{" "}
                <div className="att-progress-card-content">
                  {" "}
                  <span>Today's Target</span>{" "}
                  <div
                    className="att-progress-bar-container"
                    style={{ height: 12 }}
                  >
                    {" "}
                    <div
                  ...
                  <h1>
                    {((progressPercent / 100) * 8).toFixed(1)}{" "}
                    <span>/ 8 Hours</span>
                  </h1>{" "}
                  <span style={{ fontSize: 13, color: "#475569" }}>
                    Remaining:{" "}
                    {Math.max(8 - (progressPercent / 100) * 8, 0).toFixed(1)}{" "}
                    Hours
                  </span>{" "}
                </div>{" "}
              </div>{" "}
              <div className="att-card">
                ...
                <div className="att-calendar-grid">
                  ...
                  {["M", "T", "W", "T", "F", "S", "S"].map((d, i) => (
                    <div key={i} className="att-cal-day-header">
                      {d}
                    </div>
                  ))}{" "}
                  {calDays.map((d, i) => (
          ...
          >
            ...
            <div className="att-card">
              {" "}
              <h3 className="att-section-title">
                <Activity size={18} /> Productivity Analytics
              </h3>{" "}
            ...
            <div>
              {" "}
              <h3 className="att-section-title">
                <Fingerprint size={18} /> Quick Actions
              </h3>{" "}
              <div className="att-quick-actions-row">
                {" "}
                <div
                  className="att-action-pill"
                  onClick={() => navigate("/leave-management/apply")}
                  style={{ cursor: "pointer" }}
                >
                  {" "}
                  <FileText size={24} /> <span>Leave Request</span>{" "}
                </div>{" "}
                <div
                  className="att-action-pill"
                ...
                >
                  {" "}
                  <Calendar size={24} /> <span>Leave History</span>{" "}
                </div>{" "}
                <div className="att-action-pill">
                  {" "}
                  <Download size={24} /> <span>Download Report</span>{" "}
                </div>{" "}
                <div className="att-action-pill">
                  {" "}
                  <Timer size={24} /> <span>Overtime Form</span>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
          </div>{" "}

...
const Customers = ({ directoryOnly }) => {
  ...
  const isAdmin =
    ...
    userInfo.email === "admin@smtbms.com";
    ...
    try {
      ...
      if (directoryOnly) {
        fetchedData = fetchedData.filter(
          (c) => c.status !== "Lead" && c.customerType !== "Lead"
        );
      }
  ...
  const handleDelete = async (row) => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${row.name || row.company}?`
      )
    ) {
      try {
  ...
  const atRisk = customers.filter(
    (c) => c.status === "At Risk" || c.status === "Inactive"
  );
  const totalRevenue = customers.reduce(
    (sum, c) => sum + (Number(c.revenue) || Number(c.totalRevenue) || 0),
    0
  );
  ...
  const columns = [
    ...
    { key: "industry", label: "Industry", sortable: true },
    {
      ...
      render: (val) => {
        ...
        else if (status === "At Risk" || status === "Inactive")
          badgeClass = "danger";
  ...
  if (isAdmin) {
    actions.push({ label: "Delete", icon: Trash2, onClick: handleDelete });
  }
  return (
    <motion.div

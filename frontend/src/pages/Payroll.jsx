...
import { Search, Briefcase, CreditCard, User, FileText, Play, CheckCircle, IndianRupee } from "lucide-react";
...
const Payroll = () => {
  ...
  const totalPF = salaries.reduce((sum, s) => sum + (s.basicSalary || 0) * 0.12, 0);
  const totalTax = salaries.reduce((sum, s) => sum + (s.deductions || 0) - (s.basicSalary || 0) * 0.12, 0);
  const getStatusBadge = status => {
    if (status === "Paid") return <span className="ui-badge success">
          <div style={{
        width: 6,
        height: 6,
        borderRadius: "0px",
    ...
    if (status === "Approved") return <span className="ui-badge info">
          <div style={{
        width: 6,
        height: 6,
        borderRadius: "0px",
    ...
    if (status === "Awaiting Approval") return <span className="ui-badge warning">
          <div style={{
        width: 6,
        height: 6,
        borderRadius: "0px",
    ...
    duration: 0.4
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          ...
          <StatsCard title="Net Payroll" value={formatCurrency(totalNet, "short")} colorTheme="mint" icon={CreditCard} trendValue="After deductions" trendPositive={true} />
          <StatsCard title="Provident Fund" value={formatCurrency(totalPF, "short")} colorTheme="purple" icon={IndianRupee} trendValue="12% PF" trendPositive={true} />
          <StatsCard title="Tax & Deductions" value={formatCurrency(totalTax > 0 ? totalTax : 0, "short")} colorTheme="peach" icon={FileText} trendValue="Other deductions" trendPositive={false} />
        </StatsGrid>
        {/* Table Section */}
        <motion.div initial={{
        opacity: 0,
              ...
              width: "100%"
                ...
                <tbody>
                  {loading ? <tr>
                    ...
                    </tr> : filteredSalaries.length === 0 ? <tr>
                      <td colSpan={10} style={{
                    textAlign: "center",
                    padding: 32,
                    color: "#94a3b8"
                  }}>
                        No salary records match your criteria
                      </td>
                    </tr> : filteredSalaries.map((record, i) => {
                  const emp = record.employee || {};
                  const gross = (record.basicSalary || 0) + (record.allowances || 0);
                  return <tr key={record._id || record.id || i}>
                          <td style={{
                      fontWeight: 600,
                      ...
                      padding: "11px 12px"
                        ...
                        gap: 12
                              ...
                              <UserAvatar name={`${emp.firstName || ""} ${emp.lastName || ""}`.trim()} size={32} fontSize={12} style={{
                          flexShrink: 0
                        }} />
                              <div>
                                <div style={{
                      ...
                      textAlign: "center"
                            ...
                            {record.status === "Awaiting Approval" && <button onClick={() => handleApprove(record._id || record.id)} className="rd-btn-compact outline" style={{
                        borderColor: "#3b82f6",
                        color: "#3b82f6"
                      }}>
                                Approve
                              </button>}
                            {record.status === "Approved" && <button onClick={() => navigate(`/payroll/payment/${record._id || record.id}`)} className="rd-btn-compact primary" style={{
                        background: "#10b981"
                      }}>
                                Pay Now
                              </button>}
                            {record.status === "Paid" && <CheckCircle size={16} color="#10b981" />}
                          </td>
                        </tr>;
                })}
                </tbody>

...
const Payroll = () => {
  ...
  const totalGross = salaries.reduce(
    (sum, s) => sum + (s.basicSalary || 0) + (s.allowances || 0),
    0
  );
  const totalNet = salaries.reduce((sum, s) => sum + (s.netSalary || 0), 0);
  const totalPF = salaries.reduce(
    (sum, s) => sum + (s.basicSalary || 0) * 0.12,
    0
  );
  const totalTax = salaries.reduce(
    (sum, s) => sum + (s.deductions || 0) - (s.basicSalary || 0) * 0.12,
    0
  );
  ...
  const departments = [
    "All",
    ...new Set(salaries.map((s) => s.employee?.department).filter(Boolean)),
  ];
  const months = [
    "All",
    ...new Set(salaries.map((s) => s.month).filter(Boolean)),
  ];
    ...
    >
      <div className="rd-content">
        ...
        <StatsGrid>
          <StatsCard
            title="Gross Payroll"
            value={formatCurrency(totalGross, "short")}
            colorTheme="blue"
            icon={Briefcase}
            trendValue="Total payout"
            trendPositive={true}
          />
          <StatsCard
            title="Net Payroll"
            value={formatCurrency(totalNet, "short")}
            colorTheme="mint"
            icon={CreditCard}
          ...
          <StatsCard
            ...
            value={formatCurrency(totalTax > 0 ? totalTax : 0, "short")}
            colorTheme="peach"
            icon={FileText}
            trendValue="Other deductions"
            trendPositive={false}
          />
        </StatsGrid>
        {/* Table Section */}
                    ...
                    filteredSalaries.map((record, i) => {
                      ...
                      return (
                        <tr key={record._id || record.id || i}>
                          ...
                          <td style={{ color: "#ef4444", textAlign: "right" }}>
                          ...
                          >
                            {record.status === "Awaiting Approval" && (
                              <button
                                onClick={() =>
                                  handleApprove(record._id || record.id)
                                }

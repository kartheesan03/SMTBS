...
const warehouseIcon = createHubIcon("#3b82f6");
const yardIcon = createHubIcon("#f59e0b");
const storeIcon = createHubIcon("#10b981");
    ...
    try {
      ...
      const updatedOrders = await Promise.all(
        ordersRes.data.map(async (order) => {
          try {
            const locData = await API.get(
              `/orders/${order._id || order.id}/location`
            );
      ...
      const validOrders = updatedOrders.filter(
        (o) => o.liveLocation && o.liveLocation.lat
      );
          ...
          {loading && orders.length === 0 && locations.length === 0 ? (
            ...
            >
              ...
              {locations.map((loc, idx) => {
                const isVisible =
                  (loc.type === "Warehouse" && showWarehouses) ||
                  ...
                  >
                    <Popup maxWidth={300}>
                      ...
                      >
                        ...
                        >
                          CURRENT STOCK ({loc.materials.length})
                        </h4>
                        <div
                          style={{
                            marginBottom: "12px",
                        ...
                        >
                          SOURCED ORDERS ({loc.activeOrders?.length || 0})
                        </h4>
                        <div>
                          {loc.activeOrders && loc.activeOrders.length > 0 ? (
                            loc.activeOrders.map((order) => (
                              <div
                                key={order.id}
              ...
              {showDeliveries &&
                orders.map((order) => {
                  ...
                  const path = (order.routePath || []).map((p) => [
                    p.lat,
                    p.lng,
                  ]);
                  const sourceLoc = locations.find(
                    (l) => l.name === order.sourcedLocation
                  );
                  const sourceLine = sourceLoc
                    ? [[sourceLoc.lat, sourceLoc.lng], latLng]
                    : null;
                  return (
                        ...
                        <Popup>
                          <div style={{ minWidth: "200px" }}>
                            ...
                            {order.sourcedLocation && (
                              ...
                              >
                                Sourced from:{" "}
                                <strong>{order.sourcedLocation}</strong>
                              </div>
                            )}
                            <div
                            ...
                            >
                              <strong>Dist:</strong>{" "}
                              {order.distanceRemaining
                                ? order.distanceRemaining.toFixed(1) + " km"
                                : "-"}
                            </div>

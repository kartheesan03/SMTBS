...
const BarcodeManagement = () => {
  ...
  const registryData = materials.map((m) => {
    ...
    return {
      ...
      name: m.name,
      category:
        m.category || ["Steel", "Electronics", "Chemicals", "Tools"][hash % 4],
      quantity: m.quantity || 0,
      unit: m.unit || ["kg", "units", "liters", "pcs"][hash % 4],
      status: getStatus(m),
      barcode: m.sku
        ? `890${m.sku.replace(/\D/g, "").padStart(10, "0")}`
        : `89012345600${(m.id % 100).toString().padStart(2, "0")}`,
      qr: `SMTBMS-${m.sku || "MAT-" + m.id}-STD`,
      loc: `Store ${String.fromCharCode(65 + (hash % 4))} / Shelf ${
        (hash % 5) + 1
      }`,
      scans: hash % 50,
      scanned: hash % 2 === 0,

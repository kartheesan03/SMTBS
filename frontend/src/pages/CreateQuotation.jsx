...
const CreateQuotation = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: "",
    termsAndConditions:
      "Quotation valid until the specified date. All prices are final unless changed by mutual agreement.",
    items: [],
  });
  useEffect(() => {
  ...
  const handleAddItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { material: "", quantity: 1, unitPrice: 0, discountPercent: 0 },
      ],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const hasInvalidItem = formData.items.some(
      (i) => !i.material || !i.quantity || i.quantity < 1
    );
    if (hasInvalidItem)
      return toast.error(
        "Please ensure all items have a material and quantity >= 1"
      );
    setLoading(true);
    try {
            ...
            {formData.items.length === 0 ? (
              ...
              >
                <p style={{ color: "#6B7280", margin: 0 }}>
                  No items added yet. Click 'Add Item' to start building the
                  quote.
                </p>
              </div>
            ) : (

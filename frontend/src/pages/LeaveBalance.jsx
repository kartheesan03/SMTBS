...
const LeaveBalance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(
          "Failed to load leave balances. Make sure you have the required permissions."
        );
      } finally {
        setLoading(false);
      }
  ...
  const filtered = employeeBalances.filter((e) => {
    const nameMatch = `${e.firstName} ${e.lastName || ""}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const deptMatch = (e.department || "")
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    return nameMatch || deptMatch;
  });
  const totalStaff = employees.length;
  const totalLeavesTaken = employeeBalances.reduce(
    (acc, curr) => acc + curr.totalUsed,
    0
  );
  const avgLeavesTaken = totalStaff
    ? Math.round(totalLeavesTaken / totalStaff)
    : 0;
                        ...
                        <td>
                          <span
                            style={{
                              fontWeight: 700,
                              color:
                                emp.used.Unpaid > 0 ? "#ef4444" : "#64748b",
                              fontSize: 13,
                            }}
                          >

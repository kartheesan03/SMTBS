...
const TeamPerformance = () => {
  ...
  const perfData = employees.map((emp) => {
    ...
    const targetScore =
      totalTasks > 0
        ? Math.max(0, Math.min(100, Math.round(taskScore * 0.8)))
        : 0;
    const empAttendance = attendance.filter(
      (a) =>
        String(a.employeeId?._id || a.employeeId || a.employee) ===
        String(emp.id)
    );
    ...
    const overall =
      totalTasks > 0
        ? Math.round((taskScore + attendanceScore + targetScore) / 3)
        : 0;
    let rating = "N/A";
  ...
  const teamAvg =
    perfData.length > 0
      ? Math.round(
          perfData.reduce((sum, p) => sum + p.overall, 0) / perfData.length
        )
      : 0;
  const excellentCount = perfData.filter(
    (p) => p.rating === "Excellent"
  ).length;
  const goodCount = perfData.filter((p) => p.rating === "Good").length;
  const belowAvgCount = perfData.filter(
    (p) => p.rating === "Below Average" || p.rating === "Average"
  ).length;
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          <StatsCard
            ...
            colorTheme="blue"
            icon={TrendingUp}
            trendValue="Overall performance"
            trendPositive={teamAvg >= 75}
          />
          <StatsCard
            title="Excellent"
                    ...
                    <tr key={emp.id || i}>
                      ...
                      <td data-label="Department">
                        <span
                          style={{
                            background:
                              emp.dept === "Finance" || emp.dept === "Sales"
                                ? "#fff7ed"
                                : "#ecfdf5",
                            color:
                              emp.dept === "Finance" || emp.dept === "Sales"
                      ...
                      <td data-label="Task Score">
                        {renderMiniBar(emp.kpi, "#3b82f6")}
                      </td>
                      <td data-label="Attendance">
                        {renderMiniBar(emp.attendance, "#10b981")}
                      </td>
                      <td data-label="Target Score">
                        {renderMiniBar(emp.targets, "#8b5cf6")}
                      </td>
                      ...
                      <td
                        ...
                        data-label="Est. Appraisal"
                      >
                        {emp.appraisal}
                      </td>
                      <td data-label="Actions">
                        <button
                          onClick={() => navigate(`/employees/${emp.dbId}`)}
                          style={{
                            background: "none",
                            border: "none",
                            cursor: "pointer",

...
const Support = () => {
    ...
    const fetchTickets = async () => {
        try {
            ...
            setStats({ total: res.data.length, open, inProgress, resolved });
            setLoading(false);
        } catch (error) {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'Open': return <span className="sd-badge badge-warning">Open</span>;
            case 'In Progress': return <span className="sd-badge badge-info">In Progress</span>;
            case 'Waiting for User': return <span className="sd-badge badge-alert">Waiting for User</span>;
            case 'Resolved': 
            case 'Closed': return <span className="sd-badge badge-success">{status}</span>;
            default: return <span className="sd-badge">{status}</span>;
        }
    };
    ...
    const getPriorityBadge = (priority) => {
        switch (priority) {
            case 'High':
            case 'Critical': return <span className="sd-badge badge-danger">{priority}</span>;
            case 'Medium': return <span className="sd-badge badge-warning">Medium</span>;
            default: return <span className="sd-badge badge-info">{priority}</span>;
        }
    };
    ...
    if (selectedTicketId) {
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="service-desk-wrapper">
                <TicketDetailView 
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="service-desk-wrapper">
            <div className="sd-header">
                <div>
                    ...
                    <p>Report an issue, track your complaints, and get help from the Admin Support Team.</p>
                </div>
                <button className="btn-primary-action" onClick={() => setIsNewModalOpen(true)}>
                ...
                <div className="sd-table-container">
                    <table className="sd-table">
                        ...
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan="7" className="text-center" style={{padding: '30px'}}>Loading complaints...</td>
                                </tr>
                            ) : filteredTickets.length === 0 ? (
                                <tr>
                                    <td colSpan="7" className="text-center" style={{padding: '30px', color: 'var(--text-muted)'}}>
                                ...
                                filteredTickets.map(ticket => (
                                    <tr key={ticket._id || ticket.id}>
                                        ...
                                        <td className="text-primary truncate" style={{maxWidth: '250px'}} title={ticket.subject}>
                                            {ticket.subject}
                                        </td>

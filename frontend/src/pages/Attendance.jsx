...
const parseDateTime = (ts, base) => {
  ...
  if (
    ts.includes("T") ||
    (ts.includes("-") && ts.includes(":") && ts.length > 10)
  ) {
    const d = new Date(ts);
    if (!isNaN(d)) return d;
  }
              ...
              >
                ...
                <td style={TD}>
                  <span
                    style={{
                      fontWeight: 600,
                      color: r.checkOut ? "#dc2626" : "#94a3b8",
                    }}
                  >
...
const DailyTab = ({ myHistory }) => {
  ...
  const record = myHistory.find(
    (h) => (h.date || "").split("T")[0] === selDate
  );
  ...
  return (
    <div>
      ...
      >
        <button onClick={() => goDay(-1)} style={NAV_BTN}>
          <ChevronLeft size={15} />
        </button>
        <div
          style={{
...
const MonthlyTab = ({ myHistory }) => {
  ...
  const chartData = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    return {
      ...
      h: rec
        ? parseFloat(
            (calcHrs(rec.checkIn, rec.checkOut, rec.date) || 0).toFixed(1)
          )
        : 0,
    };
  });
  ...
  return (
    <div>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...NAV_BTN,
              color: atCurrentMonth ? "#cbd5e1" : "#64748b",
      ...
      {/* Stats */}
      <StatsGrid>
        <StatsCard title="Present" value={present} subtext="Days" />
        <StatsCard title="Absent" value={absent} subtext="Days" />
        <StatsCard title="On Leave" value={onLeave} subtext="Days" />
        <StatsCard
          title="Total Hours"
        ...
        <StatsCard
          title="Attendance %"
          value={`${pct}%`}
          trendValue={`${pct}% Attendance`}
          trendPositive={pct >= 75}
        />
      </StatsGrid>
      {/* Work Hours Chart */}
...
const Attendance = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const presentDays = thisMonth.filter(
        (h) => h.status === "Present" || h.status === "Late"
      ).length;
      const withH = hist.filter((h) => h.checkIn && h.checkOut);
      const totH = withH.reduce(
        (s, h) => s + (calcHrs(h.checkIn, h.checkOut, h.date) || 0),
        0
      );
      const avg =
        withH.length > 0 ? `${(totH / withH.length).toFixed(1)}h` : "0h";
      let streak = 0;
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          <StatsCard
            ...
            trendPositive={true}
            percentChange="2 days"
          />
          <StatsCard
            title="Avg Hours / Day"
            value={stats.avg}
            icon={Activity}
            subtext="Stable vs last month"
            trendPositive={true}

...
const ERP = () => {
  ...
  const pendingOrders = purchaseOrders.filter((o) =>
    ["Pending", "Awaiting Approval"].includes(o.status)
  );
  const approvedOrders = purchaseOrders.filter((o) =>
    ["Approved", "Processing", "In Transit", "Delivered"].includes(o.status)
  );
  ...
  const filterFunction = (o) => {
    ...
    const matchesSearch =
      !searchTerm ||
      String(o.orderNumber || o.poNumber || "")
        .toLowerCase()
      ...
      return (
        <tr key={order._id || i} style={{ height: "52px" }}>
          ...
          <td
            ...
            title={order.orderNumber || order.poNumber || order.id || "—"}
            data-label="ID"
          >
          ...
          <td
            style={{ verticalAlign: "middle", color: "#64748b" }}
            data-label="Raised"
          >
            {raised
              ? new Date(raised).toLocaleDateString("en-GB", {
                  day: "2-digit",
                  month: "2-digit",
                  year: "2-digit",
                })
          ...
          >
            {delivery
              ? new Date(delivery).toLocaleDateString("en-GB", {
                  day: "2-digit",
                  month: "2-digit",
                  year: "2-digit",
                })
          ...
          >
            ...
            <button
              ...
              title="View Order"
              onClick={() =>
                navigate(`/orders/${order._id || order.id}/tracking`)
              }
            >
              {" "}
              <Eye size={14} />{" "}
        ...
        <StatsGrid>
          ...
          <StatsCard
            ...
            value={formatCurrency(
              salesOrders.reduce(
                (sum, o) =>
                  sum + (Number(o.totalAmount) || Number(o.grandTotal) || 0),
                0
              )
            )}
            colorTheme="peach"
            ...
            <div className="rd-table-actions" style={{ flexWrap: "wrap" }}>
              ...
              {user?.role !== "Employee" && user?.role !== "Sales" && (
                <button
                  ...
                  onClick={() => navigate("/orders/select-type")}
                >
                  + Raise PO
                </button>
              )}{" "}

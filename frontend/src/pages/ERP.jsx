...
const ERP = () => {
    ...
    const filterFunction = (o) => {
        ...
        const matchesSearch = !searchTerm || 
            String(o.orderNumber || o.poNumber || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
            String(o.vendor?.name || o.vendor?.companyName || o.customer?.name || o.customer?.company || '').toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const renderTableRows = (filteredList) => {
        if (filteredList.length === 0) {
            return <tr><td colSpan={8} style={{textAlign: 'center', padding: 40, color: '#94a3b8'}}>No orders found</td></tr>;
        }
        return filteredList.map((order, i) => {
            ...
            return (
                <tr key={order._id || i} style={{ height: '52px' }}>
                    <td style={{ verticalAlign: 'middle', fontWeight: 700, color: '#3b82f6', whiteSpace: 'nowrap' }} title={order.orderNumber || order.poNumber || order.id || '—'} data-label="ID">{order.orderNumber || order.poNumber || order.id || '—'}</td>
                    <td style={{ verticalAlign: 'middle', fontWeight: 600, color: 'var(--rd-text-main)', whiteSpace: 'normal', wordBreak: 'break-word' }} title={vendorName} data-label="Vendor/Customer">{vendorName}</td>
                    <td style={{ verticalAlign: 'middle', color: '#475569', whiteSpace: 'normal', wordBreak: 'break-word' }} title={itemDesc} data-label="Item Description">{itemDesc}</td>
    ...
    return (
        ...
        >
            ...
            <div className="rd-content">
                ...
                {/* KPI Cards */}
                <StatGrid>
                    <StatCard title="Total POs" value={purchaseOrders.length} colorTheme="blue" icon={ShoppingCart} trendValue="Purchase Orders" trendPositive={true} />
                    <StatCard title="Total SOs" value={salesOrders.length} colorTheme="mint" icon={ShoppingCart} trendValue="Sales Orders" trendPositive={true} />
                    <StatCard title="Total PO Value" value={formatCurrency(totalPOValue)} colorTheme="purple" icon={DollarSign} trendValue="Total spent" trendPositive={true} />
                    <StatCard title="Total SO Value" value={formatCurrency(salesOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || Number(o.grandTotal) || 0), 0))} colorTheme="peach" icon={DollarSign} trendValue="Total revenue" trendPositive={true} />
                </StatGrid>
                ...
                >
                    ...
                    <div className="rd-table-scroll">
                        <table className="rd-table rd-table-responsive" style={{ width: '100%' }}>
                            ...
                            <tbody>
                                {renderTableRows(filteredPurchaseOrders)}
                            </tbody>
                        </table>
                    </div>
                </motion.div>
                ...
                >
                    <div className="rd-table-header" style={{borderBottom: '1px solid var(--rd-border)', flexWrap: 'wrap', gap: 16}}>
                        <div>
                            ...
                            <div className="rd-table-subtitle">All customer sales and fulfillments</div>
                        </div>
                    </div>
                    ...
                    <div className="rd-table-scroll">
                        <table className="rd-table rd-table-responsive" style={{ width: '100%' }}>
                            ...
                            <tbody>
                                {renderTableRows(filteredSalesOrders)}
                            </tbody>
                        </table>
                    </div>
                </motion.div>

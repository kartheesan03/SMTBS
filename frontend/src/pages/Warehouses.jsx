...
const Warehouses = () => {
                ...
                data.forEach(item => {
                    ...
                    if (!grouped[wh]) {
                        grouped[wh] = {
                            id: wh,
                            name: wh,
                            location: item.location || (wh === 'Unassigned' ? 'System Default' : 'Primary Location'),
                            totalItems: 0,
                            totalStock: 0,
                            totalValue: 0,
                            lowStockItems: 0,
    ...
    return (
        <div className="rd-container">
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid #e2e8f0', color: '#64748b', textAlign: 'left' }}>
                                        ...
                                        <th style={{ padding: '12px 8px', fontWeight: 600 }}>Exact Location</th>
                                    ...
                                    {selectedZone.items.map(item => {
                                        ...
                                        return (
                                            <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                                ...
                                                <td style={{ padding: '12px 8px', color: '#64748b' }}>{item.location || item.rack || item.shelf || '-'}</td>
            ...
            {showAddModal && (
                <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.4)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowAddModal(false)}>
                    <div style={{ background: '#fff', borderRadius: 0, width: '90%', maxWidth: 450, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }} onClick={e => e.stopPropagation()}>
                        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#0f172a' }}>Add New Warehouse</h3>
                            <button onClick={() => setShowAddModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: '#64748b' }}>
                                <X size={20} />
                            </button>
                        </div>
                        <div style={{ padding: '24px' }}>
                            <div style={{ marginBottom: 16 }}>
                                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 6 }}>Warehouse Name</label>
                                <input type="text" id="new-wh-name" placeholder="e.g. Facility North" style={{ width: '100%', padding: '10px 12px', border: '1px solid #cbd5e1', borderRadius: 0, outline: 'none', fontSize: 14 }} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 6 }}>Location</label>
                                <input type="text" id="new-wh-loc" placeholder="e.g. 123 Industrial Ave" style={{ width: '100%', padding: '10px 12px', border: '1px solid #cbd5e1', borderRadius: 0, outline: 'none', fontSize: 14 }} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 6 }}>Type</label>
                                <select id="new-wh-type" style={{ width: '100%', padding: '10px 12px', border: '1px solid #cbd5e1', borderRadius: 0, outline: 'none', fontSize: 14, background: '#fff' }}>
                        ...
                        <div style={{ padding: '16px 24px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                            <button onClick={() => setShowAddModal(false)} style={{ padding: '8px 16px', background: '#fff', border: '1px solid #cbd5e1', borderRadius: 0, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>Cancel</button>
                            <button onClick={() => {
                            ...
                            }} style={{ padding: '8px 16px', background: '#4f46e5', border: 'none', borderRadius: 0, fontWeight: 600, color: '#fff', cursor: 'pointer' }}>Create Warehouse</button>
                        </div>
                    </div>
                </div>
            )}

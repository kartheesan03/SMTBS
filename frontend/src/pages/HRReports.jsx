...
const HRReports = () => {
  ...
  const hrReportsList = [
    {
      ...
      format: "Excel",
      icon: <Calendar color="var(--dash-primary, #3b82f6)" />,
      color: "var(--dash-primary, #3b82f6)",
    },
    {
      name: "Employee Turnover Report",
      format: "CSV",
      icon: <Users color="var(--dash-teal, #14b8a6)" />,
      color: "var(--dash-teal, #14b8a6)",
    },
    {
      name: "Leave Utilization Audit",
      format: "CSV",
      icon: <FileText color="var(--dash-warning, #f59e0b)" />,
      color: "var(--dash-warning, #f59e0b)",
    },
    {
      name: "Payroll Disbursement Log",
      format: "CSV",
      icon: <BarChart2 color="var(--dash-purple, #8b5cf6)" />,
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to retrieve live HR analytics."
      );
    ...
    try {
      ...
      if (reportName === "Monthly Attendance Summary") {
          ...
          const empLeaves = allLeaves.filter(
            (l) =>
              ...
              (l.employee._id === emp._id ||
                l.employee.employeeId === emp.employeeId)
          );
          const taken = empLeaves.reduce(
        ...
        const rows = data.map((s) => [
          ...
          s.status || "-",
          s.paymentDate
            ? new Date(s.paymentDate).toISOString().split("T")[0]
            : "-",
        ]);
        content = generateCSV(headers, rows);
  ...
  const handleCustomSubmit = async (e) => {
    e.preventDefault();
    if (!customReport.type)
      return showToast("Please select a report category.", "error");
    ...
    try {
      ...
      const rows = [
        [
          customReport.type,
          customReport.format,
          customReport.from || "All Time",
          customReport.to || "All Time",
          new Date().toLocaleString(),
          "HR SYSTEM CORE",
        ],
      ];
      const filenameBase = `HR_Custom_${customReport.type.replace(
        /\s+/g,
        "_"
      ...
      showToast(
        `✅ Custom ${customReport.format} report compiled and downloaded!`
      );
    } catch (err) {
  ...
  const liveKPIs = {
    totalEmployees:
      stats?.hrStats?.totalEmployees ?? stats?.stats?.totalEmployees ?? 0,
    attendanceRate: stats?.hrStats?.attendanceRate ?? "98%",
    onLeave: stats?.hrStats?.onLeave ?? 0,
    newJoiners: stats?.hrStats?.newJoiners ?? 0,
  };
  const attendanceHistory =
    stats?.hrStats?.attendanceHistory &&
    stats.hrStats.attendanceHistory.length > 0
      ? stats.hrStats.attendanceHistory.map((day) => ({
          name: day.name,
          Present: day.employees,
          Rate: (
            (day.employees / (stats.hrStats.totalEmployees || 1)) *
            100
          ).toFixed(1),
        }))
      : [];
  const employeeDistribution =
    stats?.hrStats?.employeeDistribution &&
    stats.hrStats.employeeDistribution.length > 0
        ...
        <div className="custom-chart-tooltip">
          ...
          {payload.map((pld, index) => (
            <p
              key={index}
              style={{
                color: pld.color || "var(--dash-primary)",
                fontSize: "12px",
                margin: "4px 0",
                fontWeight: "bold",
              }}
    ...
    <div className="hr-reports-container">
      ...
      <header className="page-header" style={{ marginBottom: 0 }}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          <button
            className="btn-secondary-light flex-center"
      ...
      <section className="reports-grid-section">
        ...
        <div className="reports-export-grid">
          {hrReportsList.map((report, i) => (
            <div
              key={i}
              className="premium-card hr-report-card"
              style={{ padding: "24px" }}
            >
              <div
              ...
              <div className="report-details">
                <h3>{report.name}</h3>
                <p>
                  Standard Format: <strong>{report.format}</strong>
                </p>
              </div>
              <button
      ...
      <section
        className="interactive-analytics-section mt-30"
        style={{ marginTop: "30px" }}
      >
        <div className="analytics-box-header">
          <div>
            <h2 className="section-title">
              Advanced Demographic & Attendance Analytics
            </h2>
            <p className="section-desc">
              Interactive visual mappings of staff patterns and departmental
              concentrations.
            </p>
          </div>
        </div>
              ...
              {employeeDistribution.length > 0 ? (
                <ResponsiveContainer width="100%" height={260}>
                  ...
                  >
                    ...
                    <XAxis
                      dataKey="name"
                      stroke="var(--dash-text-muted, #64748b)"
                      fontSize={10}
                      tickLine={false}
                    />
                    <YAxis
                    ...
                    <Bar dataKey="Count" radius={[6, 6, 0, 0]} name="Headcount">
                      {employeeDistribution.map((entry, index) => (
                        <Cell
                          key={index}
                          fill={entry.color || "var(--dash-purple, #8b5cf6)"}
                        />
                      ))}
                    </Bar>
                  </BarChart>
      ...
      {showCustomModal && (
        <div className="cyber-modal-overlay">
          <div
            className="premium-card cyber-modal-box animate-pop"
            style={{ background: "var(--bg-card)", padding: "32px" }}
          >
            ...
            <form onSubmit={handleCustomSubmit} className="modal-form">
              <div className="field-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCustomReport({ ...customReport, type: e.target.value })
                  }
                  required
                >
                  <option value="">Choose category...</option>
                  <option value="Absence & Absence Audits">
                    Absence & Leave Utilization Audits
                  </option>
                  <option value="Workforce Demographics">
                    Workforce Demographics & Role Layout
                  </option>
                  <option value="Compliance Log">
                    Compliance & Tax Filings Log
                  </option>
                  <option value="Hiring Pipeline Stats">
                    Hiring & Onboarding Pipeline Stats
                  </option>
                </select>
              </div>
              <div className="field-row">
                <div className="field-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCustomReport({ ...customReport, from: e.target.value })
                    }
                  />
                </div>
                <div className="field-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCustomReport({ ...customReport, to: e.target.value })
                    }
                  />
                </div>
              </div>
                  ...
                  {["CSV", "XLSX", "PDF"].map((fmt) => (
                    <label
                      ...
                      className={`radio-label ${
                        customReport.format === fmt ? "active" : ""
                      }`}
                    >
                      <input
                        type="radio"
      ...
      <style jsx="true">{`
        ...
        .toast-alert.success {
          border-left: 4px solid var(--dash-success, #10b981);
          color: var(--dash-success, #10b981);
        }
        .toast-alert.error {
          border-left: 4px solid var(--dash-danger, #ef4444);
          color: var(--dash-danger, #ef4444);
        }
        .toast-alert.warning {
          border-left: 4px solid var(--dash-warning, #f59e0b);
          color: var(--dash-warning, #f59e0b);
        }
        ...
        .green-edge::before {
          background: var(--dash-success, #10b981);
        }
        .yellow-edge::before {
        ...
        @media (max-width: 640px) {
          ...
          .header-actions .btn-primary-blue {
            flex: 1;
          }
          .cyber-kpi-row {
            grid-template-columns: 1fr;

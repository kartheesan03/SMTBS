...
const CreateOrder = () => {
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { material: "", quantity: 1, price: 0 }],
    });
  };
  ...
  const handleCreateOrder = async () => {
    try {
      for (const item of formData.items) {
        const matExists = materials.find(
          (m) => String(m.id || m._id) === String(item.material)
        );
        if (!matExists) {
        ...
        <div style={{ display: "flex", gap: "20px", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              ...
              {selectedEntity.email || "No email provided"} |{" "}
              {selectedEntity.phone || "No phone provided"}
            </p>
          </div>
          <button
            type="button"
            onClick={() =>
              navigate(
                isSales ? "/erp/customers/select" : "/erp/vendors/select"
              )
            }
            style={{
              padding: "8px 16px",
                ...
                >
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const mat = materials.find(
                          (m) => String(m.id || m._id) === e.target.value
                        );
                        const newItems = [...formData.items];
                        newItems[index] = {
                          ...newItems[index],
                    ...
                    >
                      ...
                      {!isSales && materials.length === 0 ? (
                        <option value="" disabled>
                          No materials available for this vendor.
                        </option>
                      ) : (
                        materials.map((m) => (
                  ...
                  <td style={{ padding: "12px 16px", textAlign: "center" }}>
                    <button
                      type="button"
                      onClick={() =>
                        formData.items.length > 1 ? removeItem(index) : null
                      }
                      ...
                      style={{
                        ...
                        border: "none",
                        color:
                          formData.items.length === 1 ? "#cbd5e1" : "#ef4444",
                        cursor:

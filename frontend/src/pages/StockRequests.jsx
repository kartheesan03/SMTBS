...
const ArcGauge = ({ pct }) => {
  ...
  const trackD = `M ${pt(0).x} ${pt(0).y} A ${R} ${R} 0 1 1 ${trackEnd.x} ${
    trackEnd.y
  }`;
  ...
  const fillD =
    fillSweep < 1
      ? ""
      : `M ${pt(0).x} ${pt(0).y} A ${R} ${R} 0 ${largeArc} 1 ${fillEnd.x} ${
          fillEnd.y
        }`;
        ...
        <div>
          ...
          >
            ...
            <span
              style={{
                ...
                fontWeight: 700,
                background:
                  pct === 100 ? "#d1fae5" : pct >= 80 ? "#fef9c3" : "#fee2e2",
                color:
                  pct === 100 ? "#059669" : pct >= 80 ? "#a16207" : "#dc2626",
              }}
            >
              {pct === 100
                ? "✓ All Healthy"
          ...
          >
            {allCategories.map((cat) => {
              const catPct = totalItems
                ? Math.round((cat.value / totalItems) * 100)
                : 0;
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          ...
          <StatsCard
            ...
            icon={TrendingUp}
            trendValue={`${
              totalItems ? Math.round((healthyCount / totalItems) * 100) : 0
            }% within thresholds`}
            trendPositive={true}
            onClick={() => navigate("/stock-requests")}
          />
          <StatsCard
            ...
            icon={AlertTriangle}
            trendValue={`${
              totalItems ? Math.round((lowCount / totalItems) * 100) : 0
            }% approaching reorder`}
            trendPositive={false}
            onClick={() => navigate("/stock-requests")}
          />
          <StatsCard
            ...
            icon={XCircle}
            trendValue={`${
              totalItems ? Math.round((outCount / totalItems) * 100) : 0
            }% need action`}
            trendPositive={false}
            onClick={() => navigate("/stock-requests")}
          />
                    ...
                    <tr key={item.matId || i}>
                      ...
                      <td
                        style={{
                          fontWeight: 700,
                          color:
                            item.status === "Low Stock" ? "#f59e0b" : "#ef4444",
                          textAlign: "right",
                        }}
                      ...
                      <td style={{ color: "#94a3b8" }} data-label="Since">
                        {item.since}
                      </td>

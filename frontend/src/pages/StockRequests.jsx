...
const ArcGauge = ({
  ...
  const fillD = fillSweep < 1 ? "" : `M ${pt(0).x} ${pt(0).y} A ${R} ${R} 0 ${largeArc} 1 ${fillEnd.x} ${fillEnd.y}`;
  const tip = pt(fillSweep);
  const gaugeColor = pct >= 80 ? "#10b981" : pct >= 50 ? "#f59e0b" : "#ef4444";
  return <svg width="200" height="140" viewBox="0 0 200 140" aria-label={`Stock health ${pct}%`}>
      <defs>
      ...
      {/* Track */}
      <path d={trackD} fill="none" stroke="#f1f5f9" strokeWidth="12" strokeLinecap="round" />
      {/* Filled arc */}
      {fillD && <path d={fillD} fill="none" stroke={gaugeColor} strokeWidth="12" strokeLinecap="round" style={{
      filter: `drop-shadow(0 0 4px ${gaugeColor}40)`
    }} />}
      {/* Pointer dot */}
      {fillSweep >= 1 && <circle cx={tip.x} cy={tip.y} r="6" fill={gaugeColor} stroke="#fff" strokeWidth="2.5" style={{
      filter: `drop-shadow(0 1px 3px ${gaugeColor}60)`
    }} />}
      {/* Center score */}
      <text x={cx} y={cy - 14} textAnchor="middle" fontSize="32" fontWeight="800" fill="var(--rd-text-main, #1e293b)" fontFamily="inherit">
        {pct}%
      </text>
      <text x={cx} y={cy + 8} textAnchor="middle" fontSize="10" fontWeight="700" letterSpacing="1.5" fill="#94a3b8" fontFamily="inherit">
        STOCK HEALTH
      </text>
    </svg>;
};
const Sparkline = ({
  data
    ...
    bottom: 0
      ...
      <Area type="monotone" dataKey="Healthy" stroke="#10b981" strokeWidth={1.5} fill="url(#sparkGrad)" dot={false} activeDot={{
      r: 3,
      fill: "#10b981"
    }} />
    </AreaChart>
...
const SegBar = ({
  ...
  return <div style={{
    ...
    background: "#f1f5f9"
      ...
      return w > 0 ? <div key={cat.name} title={`${cat.name}: ${cat.value}`} style={{
        width: `${w}%`,
...
const StockRequests = () => {
  ...
  const alertsData = materials.filter(m => getStatus(m) !== "Healthy").map(m => ({
  ...
  return <div className="rd-container">
      <div className="rd-content">
        ...
        <StatsGrid>
          ...
          <StatsCard title="Healthy" value={healthyCount} colorTheme="mint" icon={TrendingUp} trendValue={`${totalItems ? Math.round(healthyCount / totalItems * 100) : 0}% within thresholds`} trendPositive={true} onClick={() => navigate("/stock-requests")} />
            ...
            width: "100%"
              ...
              <tbody>
                {loading ? <tr>
                  ...
                  </tr> : alertsData.map((item, i) => <tr key={item.matId || i}>
                      <td style={{
                  fontWeight: 700,
                  ...
                  color: "#3b82f6"
                      ...
                      <td style={{
                  fontWeight: 700,
                  color: item.status === "Low Stock" ? "#f59e0b" : "#ef4444",
                  textAlign: "right"
                }} data-label="Current Qty">
                        {item.current} pcs
                      </td>
                      <td style={{
                  fontWeight: 700,
                  ...
                  textAlign: "center"
                    ...
                    gap: 6
                          ...
                          {isEmployee ? <button className="rd-btn-compact primary" onClick={() => navigate("/my-materials")}>
                              Request Restock
                            </button> : <>
                              <button className="rd-btn-compact primary" onClick={() => navigate("/erp/vendors/select")}>
                                Raise PO
                              </button>

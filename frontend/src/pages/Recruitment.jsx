        ...
        >
          ...
          >
            ...
            <div>
              ...
              <select
                ...
                style={inp}
              >
                {["Open", "On Hold", "Closed", "Filled"].map((s) => (
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handle}
              rows={2}
              placeholder="e.g. 5+ years in material handling, inventory tracking software..."
              style={{ ...inp, resize: "vertical" }}
            />
          </div>
          <button
...
const Recruitment = () => {
  ...
  const moveStage = async (cand, stage) => {
    try {
      const { data } = await API.put(`/recruitment/candidates/${cand.id}`, {
        stage,
      });
      setCandidates((prev) =>
        prev.map((c) => (c.id === cand.id ? { ...c, stage } : c))
      );
      fetchAll();
    } catch {
      ...
      <div className="rd-content">
        ...
        <StatsGrid>
          <StatsCard
            ...
            colorTheme="blue"
            icon={Briefcase}
            trendValue="Active job postings"
            trendPositive={true}
          />
          <StatsCard
            title="Total Applicants"
            value={loading ? "…" : stats.totalApplied || 0}
            colorTheme="purple"
            icon={Users}
            trendValue="All applications"
            trendPositive={true}
          />
          <StatsCard
            title="In Interview"
            value={loading ? "…" : stats.interviews || 0}
            colorTheme="yellow"
            icon={Calendar}
            trendValue="Scheduled interviews"
            trendPositive={true}
          />
          <StatsCard
            title="Hired"
            value={loading ? "…" : stats.hired || 0}
            colorTheme="mint"
            icon={CheckCircle}
            trendValue="Successfully placed"
            trendPositive={true}
          />
        </StatsGrid>
        {/* ── Pipeline strip ── */}
        ...
        >
          ...
          >
            <div
              ...
              style={{ minWidth: 240, background: "#fff" }}
            >
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border:
                          stageFilter === s
                            ? `1.5px solid ${cfg?.color || "#6366f1"}`
                            : "1.5px solid #e2e8f0",
                        background:
                          stageFilter === s ? cfg?.bg || "#eef2ff" : "#fff",
                        color:
                          stageFilter === s
                            ? cfg?.color || "#4f46e5"
                            : "#64748b",
                          ...
                          <td data-label="Stage">
                            ...
                            >
                              ...
                              {stageIdx < STAGES.length - 2 && (
                                <button
                                  onClick={() =>
                                    moveStage(c, STAGES[stageIdx + 1])
                                  }
                                  title={`Move to ${STAGES[stageIdx + 1]}`}
                                  style={{
                                    background: "none",
                                    border: "none",
                                    cursor: "pointer",
                            ...
                            <div style={{ display: "flex", gap: 2 }}>
                              {[1, 2, 3, 4, 5].map((n) => (
                                <Star
                                  key={n}
                                  size={12}
                                  fill={c.rating >= n ? "#f59e0b" : "none"}
                                  color={c.rating >= n ? "#f59e0b" : "#e2e8f0"}
                                />
                              ))}
                            </div>
                          </td>

...
import { BookOpen, Users, TrendingUp, Award, Play, CheckCircle, Search, Plus, Star, Target, Layers, Code, Heart, MessageSquare, Shield, Calendar, User, Zap, X, AlertCircle } from "lucide-react";
      ...
      boxShadow: "0 20px 60px rgba(0,0,0,0.2)"
        ...
        gap: 14
          ...
          <div>
            ...
            <textarea name="description" value={form.description} onChange={handle} rows={3} placeholder="Brief course description..." style={{
            ...inp,
            resize: "vertical"
          }} />
          </div>
          ...
          gap: 12
            ...
            <div>
              ...
              <input type="color" name="color" value={form.color} onChange={handle} style={{
              ...inp,
              padding: 4,
              height: 38
            }} />
...
const TrainingDevelopment = () => {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [c, s, p] = await Promise.all([API.get("/training/courses").catch(() => ({
        data: []
      })), API.get("/training/stats").catch(() => ({
        data: {}
      })), API.get("/training/my-progress").catch(() => ({
        data: {}
      }))]);
      setCourses(c.data || []);
      setStats(s.data || {});
  ...
  const filtered = courses.filter(c => {
    ...
    const matchSrch = (c.title || "").toLowerCase().includes(searchTerm.toLowerCase()) || (c.instructor || "").toLowerCase().includes(searchTerm.toLowerCase());
                ...
                {loading ? <tr>
                  ...
                  color: "#94a3b8"
                    ...
                    marginTop: 4
                        ...
                        {courses.length === 0 ? 'Click "+ Add Course" to create your first training course.' : "Try a different category or search term."}
                      </div>
                    </td>
                  </tr> : filtered.map((course, i) => {
                const ss = STATUS_STYLE[course.myStatus] || STATUS_STYLE["Not Started"];
                const bs = course.badge ? BADGE_STYLE[course.badge] || {
                  color: "#2563eb",
                  bg: "#eff6ff"
                } : null;
                const isEnrolling = enrollingId === course.id;
                const catLabel = CATEGORIES.find(c => c.id === course.category)?.label || course.category;
                return <tr key={course.id || i}>
                        {/* Course name */}
                      ...
                      fontWeight: 600
                        ...
                        {/* Instructor */}
                        <td data-label="Instructor" style={{
                    fontSize: 13,
                    color: "#475569"
                  }}>
                          {course.instructor ? <span style={{
                      display: "flex",
                      alignItems: "center",
                      gap: 5
                    }}>
                    ...
                    color: "#64748b"
                          ...
                          {course.dueDate ? new Date(course.dueDate).toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "short",
                      year: "numeric"
                    }) : <span style={{
                      color: "#cbd5e1"
                        ...
                        <td data-label="Action">
                          <button onClick={() => course.myStatus === "Not Started" && handleEnroll(course.id)} disabled={isEnrolling || course.myStatus !== "Not Started"} style={{
                      padding: "6px 12px",
                      borderRadius: 0,
                      border: "none",
                      background: course.myStatus === "Completed" ? "#ecfdf5" : course.myStatus === "In Progress" ? "#eff6ff" : `${course.color || "#6366f1"}15`,
                      color: course.myStatus === "Completed" ? "#059669" : course.myStatus === "In Progress" ? "#2563eb" : course.color || "#6366f1",
                      fontWeight: 700,
                      fontSize: 11,

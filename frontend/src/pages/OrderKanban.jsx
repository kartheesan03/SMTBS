...
const OrderKanban = () => {
  ...
  const activeShipments = materials.filter(
    (m) => m.kanbanStatus === "In Transit"
  );
  const delayedShipments = materials.filter((m) => m.isDelayed);
  const deliveredToday = materials.filter(
    (m) => m.kanbanStatus === "Delivered"
  );
      ...
      >
        <div className="gps-modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          >
            <div>
              <h2
                style={{ margin: "0 0 4px 0", fontSize: 20, color: "#0f172a" }}
              >
                {item.name}
              </h2>
              <span
          ...
          <div style={{ padding: 24 }}>
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontWeight: 500, color: "#0f172a" }}>
                  {item.deliveryDestination || "Customer Site"}
                </div>
              </div>
              <div>
                <div
                ...
                <div
                  style={{
                    fontWeight: 600,
                    color: item.isDelayed ? "#ef4444" : "#3b82f6",
                  }}
                >
                  {item.gpsStatus}
                </div>
              ...
              <div>
                ...
                <div style={{ fontWeight: 500, color: "#0f172a" }}>
                  {item.deliveryEta
                    ? new Date(item.deliveryEta).toLocaleDateString()
                    : "Pending"}
                </div>
              </div>
            </div>
            ...
            >
              <h4
                style={{ margin: "0 0 12px 0", fontSize: 14, color: "#475569" }}
              >
                Timestamp Log
              </h4>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {item.locationUpdatedAt && (
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 500, color: "#0f172a" }}>
                      {new Date(item.locationUpdatedAt).toLocaleString()}
                    </span>
                  </div>
                )}
                {item.deliveryDispatchedAt && (
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 500, color: "#0f172a" }}>
                      {new Date(item.deliveryDispatchedAt).toLocaleString()}
                    </span>
                  </div>
                )}
                {item.deliveryCompletedAt && (
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 500, color: "#0f172a" }}>
                      {new Date(item.deliveryCompletedAt).toLocaleString()}
                    </span>
                  </div>
                )}
                {!item.locationUpdatedAt && !item.deliveryDispatchedAt && (
                  <span style={{ fontSize: 13, color: "#94a3b8" }}>
                    No timestamp data available for this stage yet.
                  </span>
                )}
              </div>
            </div>
  ...
  return (
    <div className="rd-page">
      ...
      <div className="gps-kpi-grid">
        ...
        <div
          className="rd-kpi-card"
          style={{ background: "linear-gradient(145deg, #fef2f2, #fee2e2)" }}
        >
          <div className="rd-kpi-header">
                    ...
                    <Marker
                      ...
                      icon={L.divIcon({
                        ...
                        html: `<div style="background-color: ${getPinColor(
                          item.gpsStatus
                        )}; width: 24px; height: 24px; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.2);"></div>`,
                        iconSize: [24, 24],
                        iconAnchor: [12, 12],
                      })}
                    >
                      <Popup>
                        <div style={{ padding: 4 }}>
                          ...
                          <div style={{ fontSize: 12, color: "#64748b" }}>
                            Destination: {item.deliveryDestination || "N/A"}
                          </div>
                        </div>
                      </Popup>
                    </Marker>
            ...
            <tbody>
              {filteredMaterials.map((item) => {
                let bg = "#f1f5f9",
                  color = "#64748b",
                  border = "#e2e8f0";
                if (item.gpsStatus === "In Transit") {
                  bg = "#f0fdf4";
                  color = "#16a34a";
                  border = "#bbf7d0";
                } else if (item.gpsStatus === "Signal Lost") {
                  bg = "#fef2f2";
                  color = "#ef4444";
                  border = "#fecaca";
                } else if (item.gpsStatus === "Delivered") {
                  bg = "#eff6ff";
                  color = "#3b82f6";
                  border = "#bfdbfe";
                } else if (
                  item.gpsStatus === "At Warehouse" ||
                  item.gpsStatus === "Dispatched"
                ) {
                  bg = "#fffbeb";
                  color = "#f59e0b";
                  border = "#fde68a";

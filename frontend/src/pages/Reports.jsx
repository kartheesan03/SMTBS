...
const Reports = () => {
    ...
    },
    {
      ...
      trend:
        analytics.kpis.totalRevenue > 0
          ? Math.round(
              (analytics.kpis.netProfit / analytics.kpis.totalRevenue) * 100
            )
          : 0,
  ...
  const healthMetrics = [
    {
      ...
      status:
        hm.materialHealth >= 90
          ? "Excellent"
          : hm.materialHealth >= 75
          ? "Optimized"
          : "Needs Attention",
      icon: <Box size={24} />,
      color: "#8b5cf6",
    ...
    },
    {
      ...
      status:
        hm.hrAttendanceRate >= 90
          ? "Excellent"
          : hm.hrAttendanceRate >= 75
          ? "Good"
          : "Low",
    ...
    },
    {
      ...
      status:
        hm.orderFulfillment >= 90
          ? "On Track"
          : hm.orderFulfillment >= 70
          ? "Good"
          : "Behind",
    ...
    },
    {
      ...
      status:
        hm.customerRetention >= 90
          ? "Excellent"
          : hm.customerRetention >= 70
          ? "Good"
          : "Needs Work",
      icon: <Heart size={24} />,
      color: "#ec4899",
  ...
  const exportPDF = () => {
    ...
    const trendTableData = trendData.map((t) => [
      t.name || "",
      `Rs. ${t.currentYearProfit}`,
      `Rs. ${t.lastYearProfit}`,
    ]);
    autoTable(doc, {
  ...
  const exportCSV = () => {
    ...
    trendData.forEach((row) => {
      csvRows.push(
        `${row.name},${row.currentYearProfit},${row.lastYearProfit}`
      );
    });
            ...
            <StatsGrid columns={5}>
              {kpis.map((kpi, idx) => (
                <StatsCard
                  ...
                  trendValue={
                    kpi.trend > 0
                      ? `${kpi.isUp ? "+" : "-"}${kpi.trend}% vs last period`
                      : kpi.subtitle
                  }
              ...
              {healthMetrics.map((hm, idx) => (
                <div key={idx} className="health-metric-card">
                  <div className="hm-left">
                    <div
                      className="hm-icon"
                      style={{ background: `${hm.color}15`, color: hm.color }}
                    >
                      {hm.icon}
                    </div>
                    <div className="hm-info">
                      ...
                      <div className="hm-stats">
                        ...
                        <span className="hm-status" style={{ color: hm.color }}>
                          ({hm.status})
                        </span>
                      </div>
                    </div>

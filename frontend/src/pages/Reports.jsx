...
import { Download, FileText, RefreshCw, TrendingUp, IndianRupee, Package, Users, ShoppingCart, CheckCircle, Activity, Box, UserCheck, Heart, ArrowUp, ArrowDown } from "lucide-react";
import { ResponsiveContainer, ComposedChart, AreaChart, Area, LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, PieChart, Pie, Cell } from "recharts";
...
const Reports = () => {
  ...
  const kpis = [{
    title: "Total Revenue",
    value: formatCurrency(analytics.kpis.totalRevenue, true),
    ...
    title: "Net Profit",
    value: formatCurrency(analytics.kpis.netProfit, true),
    trend: analytics.kpis.totalRevenue > 0 ? Math.round(analytics.kpis.netProfit / analytics.kpis.totalRevenue * 100) : 0,
    isUp: analytics.kpis.netProfit >= 0,
    subtitle: "Margin %",
    icon: TrendingUp,
    colorTheme: "blue"
  }, {
  ...
  const chartCyData = trendData.map(d => ({
    month: d.name,
    netProfit: d.currentYearProfit
  }));
  const chartLyData = trendData.map(d => ({
    month: d.name,
    netProfit: d.lastYearProfit
  }));
  const hm = analytics.healthMetrics;
  const healthMetrics = [{
    title: "Material Health",
    value: `${hm.materialHealth}%`,
    status: hm.materialHealth >= 90 ? "Excellent" : hm.materialHealth >= 75 ? "Optimized" : "Needs Attention",
    icon: <Box size={24} />,
    color: "#8b5cf6",
    percent: hm.materialHealth
  }, {
    title: "HR Attendance",
    value: `${hm.hrAttendanceRate}%`,
    status: hm.hrAttendanceRate >= 90 ? "Excellent" : hm.hrAttendanceRate >= 75 ? "Good" : "Low",
    icon: <UserCheck size={24} />,
    color: "#10b981",
    percent: hm.hrAttendanceRate
  }, {
    title: "Order Fulfillment",
    value: `${hm.orderFulfillment}%`,
    status: hm.orderFulfillment >= 90 ? "On Track" : hm.orderFulfillment >= 70 ? "Good" : "Behind",
    icon: <ShoppingCart size={24} />,
    color: "#3b82f6",
    percent: hm.orderFulfillment
  }, {
    title: "Customer Retention",
    value: `${hm.customerRetention}%`,
    status: hm.customerRetention >= 90 ? "Excellent" : hm.customerRetention >= 70 ? "Good" : "Needs Work",
    icon: <Heart size={24} />,
    color: "#ec4899",
    percent: hm.customerRetention
  }];

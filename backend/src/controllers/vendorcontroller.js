...
exports.getVendorById = async (req, res) => {
    try {
        ...
        // Use vendormaterial join table to find materials for this vendor
        const rows = await sequelize.query(
            `SELECT m.*, vm.supplierPrice, vm.isPreferred, vm.leadTime
             FROM vendormaterial vm
             INNER JOIN material m ON vm.materialId = m.id
             WHERE vm.vendorId = :vendorId AND vm.status = 'Active'
             ORDER BY vm.isPreferred DESC`,
            { replacements: { vendorId: Number(vendorId) }, type: sequelize.QueryTypes.SELECT }
        );

        const materials = rows.map(r => ({ ...r, _id: r.id }));
        ...
        res.status(200).json({
            vendor,
            materials,
            statistics: { totalMaterials, totalStockQty, totalInventoryValue }
        });
    } catch (error) {
...
exports.getVendorMaterials = async (req, res) => {
    try {
        ...
        console.error('[getVendorMaterials error]', error.message);
        res.status(500).json({ message: error.message });
    }
};

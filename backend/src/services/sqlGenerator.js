...
const generateAndExecuteSQL = async (user, naturalLanguageQuery) => {
    ...
    // Provide context about the database schema to the AI
    // In a real implementation, you would dynamically fetch the schema or provide a condensed version
    const schemaContext = `
        Tables:
    ...
    // Clean up if it still outputs markdown
    if (sqlQuery.startsWith('\`\`\`sql')) {
        sqlQuery = sqlQuery.replace(/^\`\`\`sql/, '').replace(/\`\`\`$/, '').trim();
    }

    // 1. Validate SQL to ensure it's safe (SELECT only)
